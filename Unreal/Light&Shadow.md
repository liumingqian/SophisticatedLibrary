...
**lightmap和AO区别**：AO应该是跟着geometry走的吧，是无方向的阴影，而且是灰色的，lightmap是彩色的，还包括光照方向等信息
...
只会生成在LightmassImportanceVolume中
...
没有lightMap的情况下会使用ILC；

lightmap coordinate Index： correct UV Channel for lightmap
...
distance field replacement mesh：假设有一个在运行时具有某些世界位置偏移的网格。通过更改网格的外观，DFAO将产生没有意义的结果。通过启用此选项（仅通过分配静态网格即可启用该选项），DFAO将使用备用静态网格来点亮其距离场AO。
